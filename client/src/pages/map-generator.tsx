...
// Create context for world map image
const WorldMapContext = createContext<{
  worldMapImage: MapGenerationResult | null;
  setWorldMapImage: (image: MapGenerationResult | null) => void;
  worldMapLoading: boolean;
  setWorldMapLoading: (loading: boolean) => void;
}>({
  worldMapImage: null,
  setWorldMapImage: () => {},
...
const useWorldMap = () => useContext(WorldMapContext);
...
export default function MapGenerator() {
  ...
  const [worldMapImage, setWorldMapImage] = useState<MapGenerationResult | null>(null);
  ...
  return (
    <WorldMapContext.Provider value={{ worldMapImage, setWorldMapImage, worldMapLoading, setWorldMapLoading }}>
      <div className="container mx-auto py-6">
        ...
        {/* Placeholder for WorldMap component */}
        <WorldMap />
      </div>
...
function WorldMap() {
  const { worldMapImage, worldMapLoading } = useWorldMap();
  ...
  return (
    <div>
      {worldMapImage && (
        <img src={worldMapImage.imageUrl} alt="World Map" style={{ maxWidth: '100%' }} />
      )}
    </div>
  );
}